...
#define ACK_TASK_PRIO (9)
#define ACK_STACK_SIZE    OS_STACK_ALIGN(80)
struct os_task ack_task;
...
ble_hci_sock_init_task(void)
{
    ...
    pstack = malloc(sizeof(os_stack_t)*ACK_STACK_SIZE);
    assert(pstack);

    os_task_init(&ack_task, "sock", ble_hci_sock_ack_handler, NULL,
                 ACK_TASK_PRIO, OS_WAIT_FOREVER, pstack, ACK_STACK_SIZE);
